...
custom_violations, flake8_violations, custom_time, flake8_time, comparison_results = benchmark_tool(
    run_custom_tool, run_flake8, file_path)

# Print results
print(f"\nCustom Tool Violations ({len(custom_violations)} found):")
for v in custom_violations:
    print(
        f"Line {v['line_number']}, Column {v['column_number']}: {v['message']}")
...
print("\nComparison Results:")
print(
    f"Violations detected only by custom tool: {len(comparison_results['custom_only'])}")
print(
    f"Violations detected only by Flake8: {len(comparison_results['flake8_only'])}")
print(
    f"Common violations detected by both: {len(comparison_results['common'])}")
